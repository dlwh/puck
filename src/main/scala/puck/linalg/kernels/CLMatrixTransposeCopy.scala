...
class CLMatrixTranposeCopy private(blockSize: Int, kernel: CLKernel) {
  ...
  def permuteTransposeCopy(dst: CLMatrix[Float],
    ...
    require(dst.rows == src.rows, s"Dst rows must match src rows, but got: ${(dst.rows,dst.cols)}, ${(src.rows,src.cols)}")
    ...
    kernel.setArgs(dst.data.createSubBuffer(CLMem.Usage.Output, dst.offset, dst.majorStride * dst.majorSize), Integer.valueOf(dst.majorStride), 
      src.data.createSubBuffer(CLMem.Usage.Input, src.offset, src.majorStride * src.majorSize), Integer.valueOf(src.majorStride), 
      intBuffer,
    ...
    val res = kernel.enqueueNDRange(queue, Array(adjustedSrcCols, 1, 1), Array(blockSize, 1, 1), (ev +: events):_*)
...
__kernel void transpose_copy(__global T* dst, int dstMajorStride, 
                             __global T* src, int srcMajorStride, __global int* srcPtrs,
