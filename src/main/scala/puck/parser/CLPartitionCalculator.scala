...
trait CLPartitionCalculator[C, L] {
  ...
  def partitions(insideTop: CLBuffer[Float], offsets: CLBuffer[Int], lengths: CLBuffer[Int], numSentences: Int, events: CLEvent*)(implicit queue: CLQueue) = synchronized {
    ...
    partitionKernel.setArgs(partitions, insideTop, offsets, lengths)
    val toWait = partitionKernel.enqueueNDRange(queue, Array(numSentences, numGrammars), Array(1, numGrammars), events:_*)
  ...
  lazy val partitionKernel = codegen.mkKernel(kernel("compute_partitions", { (partitions: Rep[Array[Float] with Global],
